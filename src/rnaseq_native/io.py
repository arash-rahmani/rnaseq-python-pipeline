...
REQUIRED_COLUMNS_MIN = ("sample", "tree", "condition")
...
def load_samples_tsv(
    ...
    require_fastq: bool = True,
) -> pd.DataFrame:
    """ Load and validate a sample sheet TSV file for the RNA-seq pipeline.

    Required columns always: sample, tree, condition
    If require_fastq=True, then also require r1 and r2 columns for FASTQ paths.
    ...
    required = REQUIRED_COLUMNS if require_fastq else REQUIRED_COLUMNS_MIN

    # Validate schema: check for required columns
    missing = [col for col in required if col not in df.columns]
    if missing:
        raise ValueError(
            f"Missing required columns in {path.name}: {missing}. Required: {required}"
        )
    ...
    if require_fastq and strict_path:
